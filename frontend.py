...
st.markdown(
    f"""
    <div style="background-color: {banner_color}; padding: 16px; border-radius: 10px; color: white; text-align: center; font-size: 22px; font-weight: bold; margin-bottom: 20px;">
        {alert_status}
    </div>
    """,
...
def get_sensor_hex(temp, vib):
    if temp > 90 or vib > 2.5:
        return "0xff2222"  # Red
...
c_a1 = get_sensor_hex(axle1_temp, axle1_vib)
c_a2 = get_sensor_hex(axle2_temp, axle2_vib)
c_a3 = get_sensor_hex(axle3_temp, axle3_vib)
c_a4 = get_sensor_hex(axle4_temp, axle4_vib)

is_locked1 = 1 if axle1_speed < 5.0 or display_color == "red" else 0
is_locked2 = 1 if axle2_speed < 5.0 else 0
is_locked3 = 1 if axle3_speed < 5.0 else 0
is_locked4 = 1 if axle4_speed < 5.0 else 0

track_speed_factor = float(v_loco_kmh) * 0.003

# -------------------------------------------------------------
# THREE.JS 3D DUAL-BOGIE UNDERCARRIAGE ARCHITECTURE
# -------------------------------------------------------------
# -------------------------------------------------------------
# THREE.JS 3D DUAL-BOGIE UNDERCARRIAGE ARCHITECTURE (FIXED)
# -------------------------------------------------------------
# -------------------------------------------------------------
# THREE.JS 3D DUAL-BOGIE DIGITAL TWIN (REAL-TIME VIBRATION FIXED)
# -------------------------------------------------------------
three_js_code = f"""
<!DOCTYPE html>
<html>
...
<body>
    <div id="hud">
        <div style="font-weight: bold; font-size: 14px; margin-bottom: 6px; color: #00d2ff;">
        ...
        <div class="axle-box">
            ...
            <span class="tag" style="background: #{c_a1[2:]};">{'LOCKED' if is_locked1 else 'OK'}</span>
        </div>
        <div class="axle-box">
            <span><strong>Axle 2 (Front):</strong> {axle2_temp}°C | {axle2_vib}G | {axle2_amp}A</span>
            <span class="tag" style="background: #{c_a2[2:]};">{'LOCKED' if is_locked2 else 'OK'}</span>
        </div>
        <div class="axle-box">
            <span><strong>Axle 3 (Rear):</strong> {axle3_temp}°C | {axle3_vib}G | {axle3_amp}A</span>
            <span class="tag" style="background: #{c_a3[2:]};">{'LOCKED' if is_locked3 else 'OK'}</span>
        </div>
        <div class="axle-box">
            <span><strong>Axle 4 (Rear):</strong> {axle4_temp}°C | {axle4_vib}G | {axle4_amp}A</span>
            <span class="tag" style="background: #{c_a4[2:]};">{'LOCKED' if is_locked4 else 'OK'}</span>
        </div>
        <div style="margin-top: 6px; color: #8a9ba8;">🖱️ Rotate: Left Click | Zoom: Scroll</div>
    </div>
    ...
    <script>
        ...
        const steelReflect = new THREE.MeshStandardMaterial({{ color: 0x888c94, metalness: 0.9, roughness: 0.2 }});
        const darkMetal = new THREE.MeshStandardMaterial({{ color: 0x181a1e, metalness: 0.7, roughness: 0.5 }});
        const railMat = new THREE.MeshStandardMaterial({{ color: 0x666a72, metalness: 0.95, roughness: 0.1 }});
        ...
        // --- 2. MAIN CONNECTING CHASSIS SPINE ---
        ...
        // --- 4. REUSABLE INDIVIDUAL AXLE ASSEMBLY ---
        function createAxle(colorHex, isLocked, tempVal, vibVal, ampVal, speedVal, posZ) {{
            ...
            const boxMat = new THREE.MeshStandardMaterial({{
                ...
                emissiveIntensity: tempVal > 70 ? 0.95 : 0.15,
                roughness: 0.2
            }});
            ...
            return {{ group, wL, wR, isLocked, vibVal, tempVal, speedVal, baseZ: posZ, particles }};
        }}

        // FRONT BOGIE AXLES
        const axle1 = createAxle("{c_a1}", {is_locked1}, {axle1_temp}, {axle1_vib}, {axle1_amp}, {axle1_speed}, -4.5);
        const axle2 = createAxle("{c_a2}", {is_locked2}, {axle2_temp}, {axle2_vib}, {axle2_amp}, {axle2_speed}, -2.2);

        // REAR BOGIE AXLES
        const axle3 = createAxle("{c_a3}", {is_locked3}, {axle3_temp}, {axle3_vib}, {axle3_amp}, {axle3_speed}, 2.2);
        const axle4 = createAxle("{c_a4}", {is_locked4}, {axle4_temp}, {axle4_vib}, {axle4_amp}, {axle4_speed}, 4.5);
        ...
        function animate() {{
            ...
            // Rotate Wheels & Apply Dynamic Real-Time Vibration
            axlesList.forEach((ax, idx) => {{
                ...
                // ✅ ALWAYS SHAKE IF VIBRATION G-FORCE > 0.5G
                if (ax.vibVal > 0.5) {{
                    const shakeFactor = ax.vibVal * 0.006;
